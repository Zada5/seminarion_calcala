...
data/processed/first_cleaning/*.csv         data/processed/first_cleaning/*.csv
        ...
        +------- manual review, harmonization, class labels -------+
          ...
          |                         |                                             |
          v                         v                                             v
 descriptive R step        scripts/05_event_study_0710.R                              scripts/06_did_0710.R
          |                         |                                             |
          v                         v                                             v
...
The generator is intentionally simple but keeps the placebo clean. It reads the real event timeline, converts real event dates to Sunday-start event weeks, removes every candidate placebo week within `3` weeks of any real event week, then samples one distinct Sunday-start placebo week for each real event with seed `20260510` from the remaining clean pool. In the current v4 data this leaves `233` clean candidate weeks from the allowed date pool `2020-01-26` through `2025-12-07`. The script then assigns placebo labels matching the real event type counts: `7` political and `5` terror/security.
...
2. Verify `data/generated/placebo_events_2020_2025.csv` has the same row count and political/terror split as the current real event timeline, unique Sunday dates, dates inside `2020-01-26` through `2025-12-07`, and minimum distance `> 3` weeks from every real event week. With v4, this is `12` rows: `7` political and `5` terror/security.
...
4. Run `Rscript scripts/05_event_study_0710.R` to refresh placebo correlations, placebo event-study outputs, real-vs-placebo correlation tables, event-study tables (`.csv`, `.md`, `.html`, `.tex`, `.xlsx` where applicable), figures, and the main regression summary.
5. Run `Rscript scripts/06_did_0710.R` to refresh placebo DiD outputs, placebo DiD paper tables, real-vs-placebo DiD tables, DiD tables (`.csv`, `.md`, `.html`, `.tex`, `.xlsx`, `.png`, `.pdf` where applicable), figures, and the DiD regression summary.
...
The same descriptive outputs are also generated by the descriptive block inside `scripts/05_event_study_0710.R`. If a standalone descriptive R script is used, it should consume the same preferred weekly inputs in `data/processed/second_cleaning/` and write to this same folder.
...
| Preferred weekly files | `data/processed/second_cleaning/weekly_party_spend_*.csv` | manual second cleaning after first weekly conversion | `scripts/05_event_study_0710.R`, `scripts/06_did_0710.R` |
| Real event timeline | `data/raw/events/Consolidated List of Terror and Political incidents 2020-2025 v4.csv` | manual consolidated event list | `scripts/05_event_study_0710.R`, `scripts/06_did_0710.R` |
...
| Placebo event list | `data/generated/placebo_events_2020_2025.csv` | `scripts/03_generate_placebo_events.py` | `scripts/05_event_study_0710.R`, `scripts/06_did_0710.R` |
| Descriptive outputs | `outputs/analysis/descriptive/*.csv` | dedicated descriptive R step / descriptive block in `scripts/05_event_study_0710.R` | data hygiene review, seminar descriptive tables |
| Event-study outputs | `outputs/analysis/**` | `scripts/05_event_study_0710.R` | seminar tables, figures, interpretation |
| DiD outputs | `outputs/did/**` | `scripts/06_did_0710.R` | seminar tables, figures, interpretation |
