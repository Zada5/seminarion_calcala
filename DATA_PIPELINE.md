...
The generator is intentionally simple: it samples `66` distinct Sunday-start weeks with seed `20260510` from the allowed date pool `2020-01-26` through `2025-12-07`, which is three weeks after the first Sunday week of 2020 and three weeks before the last Sunday week of 2025. It then assigns `36` political and `30` terror placebo labels.

Both analysis scripts read this root file directly. They do not write separate expanded placebo-date tables. If the file is missing or malformed, the scripts stop and tell the user to regenerate it.
...
2. Verify `placebo_events_2020_2025.csv` has exactly `66` rows, `66` unique Sunday dates, `36` political rows, `30` terror rows, and dates inside `2020-01-26` through `2025-12-07`.
3. Run `Rscript event_study_0710.R` to refresh placebo correlations, placebo event-study outputs, real-vs-placebo correlation tables, and the main regression summary.
4. Run `Rscript did_0710.R` to refresh placebo DiD outputs, placebo DiD paper tables, real-vs-placebo DiD tables, and the DiD regression summary.
5. Confirm there is still only one placebo-date table: `placebo_events_2020_2025.csv`. No output file named `placebo_events_dates.csv` should be recreated.
6. Review `git status --short` and make sure all expected placebo-dependent outputs changed together. If the date range, counts, or seed changes, update this file, `README.md`, and `AGENTS.md` in the same commit.
