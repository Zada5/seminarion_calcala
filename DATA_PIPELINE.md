...
- The R scripts force a UTF-8 locale on startup so Hebrew column names and cell values in the descriptive/Table 1 outputs (`outputs/analysis/tables/sample_statistics_he.*`, including the Hebrew PNGs) write as real UTF-8 bytes and render correctly. If the script is invoked from a shell where `LC_ALL`/`LANG` is unset (locale = `C`), Hebrew strings would otherwise be tagged `Encoding="unknown"` and `readr::write_csv` / `ragg::agg_png` would escape them to `<XX>` byte sequences or empty glyphs. The fix lives at the very top of `scripts/05_event_study.R` and tries `en_US.UTF-8`, then `C.UTF-8` as fallback.
