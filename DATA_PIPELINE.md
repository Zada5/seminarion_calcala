...
The generator is intentionally simple but keeps the placebo clean. It randomly draws distinct Sunday-start weeks with seed `20260510` from the allowed date pool `2020-01-26` through `2025-12-07`, rejecting any draw within `3` weeks of a real event week. It writes `60` placebo events using a `7:5` political-to-security ratio: `35` political rows and `25` terror/security rows. Security placebo rows are stored as `terror` because that is the downstream analysis bucket name.
...
2. Verify `data/generated/placebo_events_2020_2025.csv` has exactly `60` rows, `60` unique Sunday dates, `35` political rows, `25` terror/security rows, dates inside `2020-01-26` through `2025-12-07`, and minimum distance `> 3` weeks from every real event week.
