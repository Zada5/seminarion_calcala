...
This is the canonical placebo event-week input. It is generated by:

```bash
python3 generate_placebo_events.py
```

The generator parses the real event timeline with the same date and type rules as the R scripts, counts the usable real events, and writes the same number of placebo rows. In the current seminar window this is `66` placebo events: `36` political and `30` terror. The generated dates are distinct Sunday-start weeks sampled with seed `20260510` from `2020-01-26` through `2025-12-07`, which is three weeks after the first Sunday week of 2020 and three weeks before the last Sunday week of 2025. The script also enforces that placebo weeks are strictly more than three weeks away from every real event week.

Both analysis scripts read this file when present. If it is missing, the scripts fall back to deterministic seeded placebo generation using the same seed and boundary-buffer rule, but the repository version should keep this file so collaborators can inspect the exact placebo dates.
...
`analysis_outputs/placebo_events/placebo_events_dates.csv` is the analysis-expanded copy of the root placebo file. It adds the columns used internally by the R script (`event_type_raw`, `event_name`, `event_id`, and `event_week_start_sunday`) and should refresh whenever `placebo_events_2020_2025.csv` changes.
...
`analysis_outputs_did/placebo/post_from_0/placebo_events_dates.csv` is the DiD-expanded copy of the same canonical root placebo file and should match the event-study expanded copy after both scripts are rerun.
...
| Placebo event generator | `generate_placebo_events.py` | maintained script | `placebo_events_2020_2025.csv` |
