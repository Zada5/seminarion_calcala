...
- `generate_placebo_events.py`: regenerate the canonical random placebo event-week file from the real event timeline
...
- Placebo dates are explicit in `placebo_events_2020_2025.csv` and are generated by `python3 generate_placebo_events.py`. The current file has 66 events, matching the real timeline count and type mix (36 political, 30 terror). It samples distinct Sunday weeks with seed `20260510` from `2020-01-26` through `2025-12-07`, keeps them strictly more than three weeks away from real event weeks, and leaves a full three-week buffer from the analysis-window edges.
- Both analysis scripts read this file when present, validate required columns (`event_date`, `event_type_group`), validate `event_type_group` is `political` or `terror`, check that placebo weeks are more than `analysis_window_weeks + 1` weeks away from real event weeks, and keep placebo weeks far enough inside the seminar sample to have a complete `+/-2` regression window.
- If `placebo_events_2020_2025.csv` is missing, `event_study_0710.R` and `did_0710.R` fall back to deterministic random placebo generation with seed `20260510`, sampling Sunday weeks inside the 2020-01-05 through 2025-12-28 seminar window with a three-week edge buffer for the default `+/-2` event-study window.
