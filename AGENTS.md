...
- Placebo dates live in exactly one canonical table: `placebo_events_2020_2025.csv`. It is generated by `python3 generate_placebo_events.py`, which samples 66 distinct Sunday weeks with seed `20260510` from the allowed pool `2020-01-26` through `2025-12-07`, then assigns 36 political and 30 terror labels.
...
- If changing the placebo strategy, follow the "Placebo refresh checklist" in `DATA_PIPELINE.md`: regenerate `placebo_events_2020_2025.csv`, verify count/type/date constraints, rerun `event_study_0710.R`, rerun `did_0710.R`, confirm no `placebo_events_dates.csv` copy was recreated, and update `DATA_PIPELINE.md`, this guide, and `README.md` if counts/range/seed change.
