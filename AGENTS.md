...
- `analysis_outputs/event_study_coefs_0710_all_party_org.csv` (same split coefficients, named for readability)
...
- `analysis_outputs_did/did_coefs_0710_all_party_org.csv` (same split coefficients, named for readability)
...
- October 7, 2023 is kept both as an all-entity dedicated model and as split models for political parties and other organizations/people. Do not collapse this back into only one combined result.
...
- Generated CSV outputs are rounded to 3 decimal places through `write_clean_csv()` / `round_numeric_columns()` for readability and to avoid confusing complex-number formatting in outputs.
