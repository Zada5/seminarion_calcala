...
The generator writes `placebo_events_2020_2025.csv` with the same number and type mix as the real event timeline. In the current seminar window this is 66 placebo events: 36 political and 30 terror. Placebo dates are distinct Sunday-start weeks sampled with seed `20260510` from `2020-01-26` through `2025-12-07`, and are kept strictly more than three weeks away from every real event week.
...
It now also runs the same placebo-date design used by `event_study_0710.R`, using `placebo_events_2020_2025.csv` when available and otherwise falling back to the seeded deterministic placebo generator with the same three-week boundary buffer.
...
* Uses the repository placebo list file `placebo_events_2020_2025.csv` (if present) so placebo dates are explicit and shared across collaborators. That file is generated by `generate_placebo_events.py`, keeps the same real-event count and political/terror mix, and samples only Sunday weeks with the three-week edge buffer needed for the full `+/-2` regression window.
...
| Placebo event generation | Real event timeline | `python3 generate_placebo_events.py` | `placebo_events_2020_2025.csv` |
