...
- `scripts/05_event_study.R`: main analysis script (descriptive stats + regressions + figure)
- `scripts/06_did.R`: separate DiD-style post-event regression script on the same cleaned weekly inputs
...
5. Run `scripts/05_event_study.R` against second-cleaning files.
6. Optionally run `scripts/06_did.R` on the same cleaned files for separate post-event FE estimates. This now includes placebo DiD on the canonical placebo weeks.
...
`scripts/05_event_study.R` uses these defaults if no command-line args are supplied:
...
`scripts/06_did.R` uses the same default input files, with:
...
- Correlation summaries are weekly aggregate correlations, not per-event correlations. `scripts/05_event_study.R` builds weekly event counts for `all_events`, `political`, and `terror`, then correlates those counts with total weekly spend for `all_entities`, `political_party`, and `other_org_or_person`.
...
- Placebo dates live in exactly one canonical table: `data/generated/placebo_events_2020_2025.csv`. It is generated by `python3 scripts/03_generate_placebo_events.py`, which randomly draws 75 distinct Sunday weeks with seed `20260510` from the allowed pool `2020-01-26` through `2025-12-07`, rejecting weeks within 3 weeks of any real event week. It uses a 16:9 security-to-political ratio: 48 terror/security rows and 27 political rows. Security placebo rows are stored as `terror` because that is the downstream analysis bucket name.
...
- October 7, 2023 is kept both as an all-entity dedicated model and as split models for political parties and other organizations/people. Do not collapse this back into only one combined result. The canonical split coefficient files are `event_study_coefs_oct7_by_group.csv` and `did_coefs_oct7_by_group.csv` inside the relevant October 7 output subfolders; do not recreate the old duplicate `*_oct7_all_party_org.csv` aliases.
...
- Do not add manually created report files at the repository root. The only legacy October 7 compatibility files intentionally kept live under `outputs/oct7_legacy/` and must be generated by `scripts/05_event_study.R`.
- DiD-style analysis stays in `scripts/06_did.R` and writes to `outputs/did/`. Do not merge DiD outputs into the main `outputs/analysis/` folder unless the project owner explicitly changes the reporting structure.
- `scripts/06_did.R` mirrors the placebo-date logic from `scripts/05_event_study.R`: it reads and validates `data/generated/placebo_events_2020_2025.csv` as the only placebo-date source and writes side-by-side real/placebo DiD comparison tables under `outputs/did/tables/`.
...
- In `scripts/06_did.R`, `PostEvent = 1` when `relative_week >= 0` within the chosen event window.
