...
write_paper_style_header <- function(summary_connection,
  ...
  writeLines(strrep("=", rule_width), con = summary_connection)
  writeLines("Generated by event_study_0710.R from the input files listed in this run.", con = summary_connection)
  writeLines("", con = summary_connection)
...
# Legacy root-level outputs kept for backward compatibility with earlier
# seminar drafts. These are generated by the script so they remain reproducible;
# the richer canonical outputs live under analysis_outputs/.
if (nrow(oct7_event) == 1 && exists("oct7_event_window") && nrow(oct7_event_window) > 0) {
  ...
  readr::write_csv(legacy_data_window, "data_window.csv", na = "")
}
...
if (nrow(oct7_coefficients_ref_minus1_for_plot) > 0) {
  ...
  readr::write_csv(legacy_oct7_coefficients, "event_study_coefs.csv", na = "")
}

if (!is.null(oct7_ref_minus1_plot)) {
  ggplot2::ggsave(
    filename = "event_study_figure.png",
    plot = oct7_ref_minus1_plot,
    width = 9,
    height = 5,
    dpi = 300
...
if (!is.null(oct7_model_ref_minus1)) {
  ...
  writeLines(legacy_summary_lines, con = "regression_summary.txt")
}
